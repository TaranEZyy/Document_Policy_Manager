...
import {
  setFolderData as setFolderDataRedux,
  setExpandedPaths as setExpandedPathsRedux,
  setSelectedDocument as setSelectedDocumentRedux,
  setAllFiles as setAllFilesRedux,
  setArchivedBoolean as setArchivedBooleanRedux,
  setAllVersionsOfSelectedDocument as setAllVersionsRedux,
  setCurrentFilePath as setCurrentFilePathRedux,
...
const DocumentHierarchy = () => {
  ...
  const buildNestedStructure = (folders) => {
    ...
    const buildTree = (folderId) => {
      ...
      folder.childrenIds.sort((a, b) => folderMap[a]?.name.localeCompare(folderMap[b]?.name))
        .forEach(childId => {
          const childNode = buildTree(childId);
          if (childNode) nestedFolder.subfolders[childNode.id] = childNode;
        });
      delete nestedFolder.childrenIds;
      return nestedFolder;
    };

    return rootFoldersArray.sort((a, b) => a.name.localeCompare(b.name))
      .map(rootFolder => buildTree(rootFolder.id))
      .filter(Boolean);
  };

  const toggleExpand = (folderId) => {
    dispatch(setExpandedPathsRedux({
      ...expandedPaths,
                      ...
                      <div className="dropdown-menu">
                        ...
                        <button
                          ...
                          onClick={async (e) => {
                            ...
                            if (confirmDelete) {
                              try {
                                await axios.delete(`${apiBaseUrl}/folders/${folderObj.id}`);
                                toast.success('Folder deleted successfully!');
                                setOpenMenuForFolderId(null);
                                setSelectedDocument(null); // Clear selected document if it was in the deleted folder
                                fetchFolderAndFileData();
                              } catch (error) {
            ...
            {isOpen && (
              <ul>
                ...
                {folderObj.subfolders &&
                  renderFolder(Object.values(folderObj.subfolders), fullPath)}
              </ul>
            )}
          </li>
        );
  ...
  <div
   ...
   onClick={() => toggleExpand(`virtual-archived-${code}`)}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexGrow: 1 }}>
      <FaFolder style={{ color: 'red', fontSize: '1.3rem' }} /> {/* Changed to red for orphaned */}
      {displayFolderName} 
      {/* (Orphaned) */}
    </div>
  </div>
    ...
    <div className={`document-hierarchy `}>
      ...
      {isLoading ? (
        ...
        <LeftPanel
          ...
          archivedBoolean={archivedBoolean} // Pass archivedBoolean to LeftPanel
          setArchivedBoolean={setArchivedBoolean}
        />
                ...
                <input
                  ...
                  onKeyDown={(e) => { // This onKeyDown is on the INPUT FIELD
                    if (e.key === 'Enter') {
                      ...
                      if (isRenaming) {
                        axios.patch(`${apiBaseUrl}/folders/${folderIdToRename}/name?newName=${encodeURIComponent(newFolderName)}`)
                          .then(() => {
